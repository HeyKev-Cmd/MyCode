...
df=pd.read_csv('108_ab107_S.csv',encoding='ANSI')
dt1=df[df['學校類型']!=''  ]
dt2=dt1  [[ '學位生_正式修讀學位外國生','學校名稱','學校類型']]
bins=[-1]
labels=[]
for x in range(300,max(dt2['學位生_正式修讀學位外國生'])+1 ,300  ):
      bins.append(x)
      labels.append(str(x-300)+"-"+  str(x)  )
...
# #設定圖表大小
fig=plt.figure(figsize=[len(count),15]) #繪製直條圖
plt.xlabel('學生數量區間')
plt.ylabel('學校數量')
